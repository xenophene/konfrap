...
k.initGraph = function () {
	...
	var draw = function (json) {
		...
		node.append('image')
					...
					.attr("max-width", 20);
									//.attr('r', 5)
									//.style('fill', function (d) { return k.fill(d.group); });
		...
		node.append('text')
				.attr('dx', 12)
				.attr('dy', '.35em')
				.text(function (d) { return 'sdfjkdlsfja'; });
				
		
		k.vis.style("opacity", 1e-6)
			.transition()
			.duration(1000)
			.style("opacity", 1);
